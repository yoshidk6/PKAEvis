...
You can control graph appearance, such as ggplot2 theme (`ggtheme`), Y-axis range (`ggtheme`), legend title (`ae.col.name`), or colors of AE curves (`ae.palette`):
...
You can also directly edit each component in the list object returned from `pkaeplot` function and then use the edited item list as an input to `combine_pkaeplot` function.
